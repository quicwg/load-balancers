...
- A 96 bit token sequence number transmitted in clear text, but
...
The 96 but token sequence number is set to a random value
using a cryptography grade random number
generator. 

The token key identifier and the corresponding
AEAD key and AEAD IV are provisioned by the server. The server also
