...
Before doing so, it's a good idea to familiarize yourself with our current [issues list](https://github.com/quicwg/load-balancers/issues) and [charter](https://datatracker.ietf.org/wg/quic/about/). If you're
...
When a new draft is published, the design issues that have been closed since the last draft will be highlighted on the mailing list, to aid reviewers. Once consensus is confirmed, those issues will be labeled with [`has-consensus`](https://github.com/quicwg/load-balancers/issues?utf8=✓&q=label%3Ahas-consensus%20).
...
* [`arch`](https://github.com/quicwg/load-balancers/labels/arch): The issue is a higher-level architectural issue that should drive the solution to a number of other issues.
* [`needs-discussion`](https://github.com/quicwg/load-balancers/labels/needs-discussion): The issue blocks progress to our next milestone.
* [`editor-ready`](https://github.com/quicwg/load-balancers/labels/editor-ready): The Working Group believes it has a viable resolution, but the editors need to incorporate that into the document so we can see it in situ.
